...
const debug = debugModule("socket.io-valkey");

function looksLikeASocketId(room: any) {
  return typeof room === "string" && room.length === 20;
}

export interface ShardedValkeyAdapterOptions {
  /**
   * The prefix for the Valkey Pub/Sub channels.
   *
   * @default "socket.io"
   */
  channelPrefix?: string;
  /**
   * The subscription mode impacts the number of Valkey Pub/Sub channels:
   *
...
/**
 * Create a new Adapter based on Valkey sharded Pub/Sub (introduced in Redis 7.0 and supported by Valkey).
 *
 * @see https://valkey.io/topics/pubsub/
 *
 * @param pubClient - the Valkey client used to publish
 * @param subClient - the Valkey client used to subscribe
...
export function createShardedAdapter(
  ...
  return function (nsp) {
    return new ShardedValkeyAdapter(nsp, pubClient, subClient, opts);
  };
}

class ShardedValkeyAdapter extends ClusterAdapter {
  private readonly pubClient: any;
  private readonly subClient: any;
  private readonly opts: Required<ShardedValkeyAdapterOptions>;
  private readonly channel: string;
  private readonly responseChannel: string;

  constructor(nsp, pubClient, subClient, opts: ShardedValkeyAdapterOptions) {
    super(nsp);
