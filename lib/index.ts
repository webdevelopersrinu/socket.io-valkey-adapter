...
const debug = require("debug")("socket.io-valkey");
...
export interface ValkeyAdapterOptions {
  /**
  ...
  /**
   * The parser to use for encoding and decoding messages sent to Valkey.
...
/**
 * Returns a function that will create a ValkeyAdapter instance.
 *
 * @param pubClient - a Valkey client that will be used to publish messages
 * @param subClient - a Valkey client that will be used to receive messages (put in subscribed state)
...
export function createAdapter(
  pubClient: any,
  subClient: any,
  opts?: Partial<ValkeyAdapterOptions>
) {
  return function (nsp) {
    return new ValkeyAdapter(nsp, pubClient, subClient, opts);
  };
}

export class ValkeyAdapter extends Adapter {
  public readonly uid;
  ...
  private valkeyListeners: Map<string, Function> = new Map();
  ...
  constructor(
    ...
    const usesCallbackApi = typeof this.pubClient.pSubscribe === "function";
    if (usesCallbackApi) {
      this.valkeyListeners.set("psub", (msg, channel) => {
        this.onmessage(null, channel, msg);
      });

      this.valkeyListeners.set("sub", (msg, channel) => {
        this.onrequest(channel, msg);
      });

      this.subClient.pSubscribe(
        this.channel + "*",
        this.valkeyListeners.get("psub"),
        true
      );
      this.subClient.subscribe(
        ...
        this.valkeyListeners.get("sub"),
        true
      );
    } else {
      this.valkeyListeners.set("pmessageBuffer", this.onmessage.bind(this));
      this.valkeyListeners.set("messageBuffer", this.onrequest.bind(this));

      this.subClient.psubscribe(this.channel + "*");
      this.subClient.on(
        "pmessageBuffer",
        this.valkeyListeners.get("pmessageBuffer")
      );
      ...
      this.subClient.on(
        "messageBuffer",
        this.valkeyListeners.get("messageBuffer")
      );
    }

    this.friendlyErrorHandler = function () {
      if (this.listenerCount("error") === 1) {
        console.warn("missing 'error' handler on this Valkey client");
      }
    };
  ...
  close(): Promise<void> | void {
    ...
    if (usesCallbackApi) {
      ...
      // Some callback-based clients have a bug when unsubscribing multiple channels at once, so we'll unsub one at a time.
